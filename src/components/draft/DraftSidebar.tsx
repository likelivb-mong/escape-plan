    ...
    <div className="flex flex-col h-full border-l border-white/[0.06]">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-0 divide-y divide-white/[0.05]">
        ...
        <SidebarSection>
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-white/85">{status.completenessScore}<span className="text-xs text-white/30 font-normal ml-0.5">/ 100</span></p>
            ...
            <ul className="mt-3 flex flex-col gap-2">
              {nextSteps.map((step, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="flex-shrink-0 mt-[3px] text-white/20 text-[10px]">→</span>
                  <p className="text-[10px] text-white/45 leading-relaxed">{step}</p>
                </li>
              ))}
            </ul>
          </SidebarSection>
              ...
              {doc.adoptedPuzzlesByStage.map((s) => (
                <div key={s.stageKey} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-white/50">{s.stageLabel} — {s.stageTitle}</span>
                  </div>
                  <span className="text-[10px] text-white/30 tabular-nums">{s.puzzles.length}개</span>
                </div>
              ))}
            </div>
          </SidebarSection>
      ...
      <div className="flex-shrink-0 px-4 py-4 border-t border-white/[0.06] flex flex-col gap-2">
        <button
          ...
          className="w-full py-2 rounded-xl bg-white/90 text-black text-xs font-semibold hover:bg-white active:bg-white/80 transition-colors"
        >
          Save Draft
        </button>
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-2 rounded-xl border border-white/[0.10] text-[11px] text-white/40 hover:border-white/20 hover:text-white/60 transition-all"
          >
            Export PDF
          </button>
          <button
...
function CheckItem({ done, label }: { done: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className={`w-3.5 h-3.5 rounded flex-shrink-0 flex items-center justify-center text-[8px] ${done ? 'bg-emerald-400/20 text-emerald-300/80' : 'border border-white/[0.10] text-white/20'}`}>
        {done ? '✓' : ''}
      </span>
      <span className={`text-[10px] ${done ? 'text-white/55' : 'text-white/25'}`}>{label}</span>
    </div>
  );
}
...
function StatPill({
  ...
  return (
    <div className="flex flex-col px-3 py-2.5 rounded-xl border border-white/[0.07] bg-white/[0.015]">
      <span className="text-[9px] text-white/20 font-bold uppercase tracking-wider mb-0.5">{label}</span>
      <span className={`text-[12px] font-bold tabular-nums ${valCls}`}>{value}</span>
    </div>
  );
}
