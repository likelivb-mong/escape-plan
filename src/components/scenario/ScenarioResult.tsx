    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.025] overflow-hidden">
        <div className="px-5 py-3 border-b border-white/[0.05] flex items-center justify-between">
          ...
          <button
            onClick={handleCopy}
            className="px-3 py-1 rounded-lg border border-white/10 text-[10px] font-medium text-white/40 hover:text-white/70 hover:border-white/25 transition-all"
          >
            {copied ? '복사됨!' : '전체 복사'}
          </button>
        </div>
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.025] overflow-hidden">
        ...
        <div className="px-5 py-4">
          <p className="text-[13px] text-white/70 leading-relaxed">{result.scenarioText}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/20 outline-none focus:border-white/30 transition-all resize-none"
        />
      </div>
    </div>
  );
...
function SummaryRow({ label, value, badgeStyle }: { label: string; value: string; badgeStyle: string }) {
  return (
    <div className="flex items-start gap-2">
      <span className={`flex-shrink-0 px-2 py-0.5 rounded-md border text-[9px] font-medium mt-0.5 ${badgeStyle}`}>
        {label}
      </span>
      <p className="text-[11px] text-white/45 leading-relaxed">{value}</p>
    </div>
  );
}
