...
export default function ChatPage() {
  ...
  // ── 출근하기 ──────────────────────────────────────────────────────────────
  // 버그 수정: fetchMessages 제거 — Realtime 구독이 시스템 메시지를 받아 state에 추가함
  // fetchMessages로 덮어쓰면 Realtime 이벤트와 경합하여 메시지가 2번 표시될 수 있음
  const handleClockIn = async () => {
    if (!user || !selectedRoomId || isClocking) return;
  ...
  const handleClockOut = async () => {
    if (!user || !workStatus || isClocking) return;
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm z-10">
                  <div className="flex flex-col items-center gap-4 text-center px-8">
                    ...
                    <button
                      ...
                      disabled={isClocking}
                      className="mt-1 px-5 py-2 rounded-xl text-xs font-semibold bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/25 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    >
